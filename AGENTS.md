...
- Prefer light typography across the site. Avoid introducing heavy font weights unless there is a deliberate hierarchy need; homepage/news links and metadata should follow the lighter site-wide feel.
- The homepage News block is a custom CSS grid in `assets/scss/template.scss` selected with `.wg-collection:has(.homepage-news-marker)`. Keep "See all events" at the bottom of the second desktop column, and let it flow normally in the single-column mobile layout.
